...
const SYSTEM_PROMPT = {
    role: "system",
    content: `You are Student Hub AI.
...
app.post("/api/chat", auth, upload.array("files", 5), async (req, res) => {
    ...
    try {
        ...
        // Fetch recent context from DB per user to maintain conversational continuity
        const pastChats = await Chat.find({ userId })
            .sort({ createdAt: -1 })
            .limit(MAX_HISTORY_MESSAGES);

        const formattedHistory = pastChats.reverse().map(c => ({
            role: c.role,
            content: c.content
        }));
        ...
        await Chat.insertMany([
            { userId, role: "user", content: message || "[File Upload]" },
